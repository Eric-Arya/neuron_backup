...
The source dataset is the official AdvBench `harmful_behaviors.csv`. The local copy at `/workspace/xcy/dataset/shared/advbench/raw/harmful_behaviors.csv` is byte-for-byte identical to the upstream file and contains 520 `goal,target` rows.
...
```bash
cd /workspace/xcy/safety_repro/iclr_neuron/neuron_deactivate
/workspace/xcy/miniconda3/envs/iclr_neuron_deactivation/bin/python \
