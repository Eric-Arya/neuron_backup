...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x, early_layers):
        ...
        if early_layers:
            up_score = torch.sum(torch.abs(up_proj), dim=1)
            down_column_norm = torch.sum(torch.abs(self.down_proj.weight), dim=0)
            down_score = torch.sum(torch.abs(intermediate_states) * down_column_norm, dim=1)
            return real_output, up_score.squeeze().tolist(), down_score.squeeze().tolist()
...
class LlamaAttention(nn.Module):
    ...
    def shared_forward(
        ...
        **kwargs,
        ...
        # K/V are shared by groups of query heads in Llama 3. Keep their
        # importance in the native KV-projection space so each physical
        # channel is represented exactly once in detector output.
        kv_heads = self.num_key_value_heads
        q_per_kv = self.num_key_value_groups
        q_grouped = query_states.view(bsz, kv_heads, q_per_kv, q_len, self.head_dim)
        logits_grouped = attn_logits.view(bsz, kv_heads, q_per_kv, q_len, -1)
        weights_grouped = attn_weights.view(bsz, kv_heads, q_per_kv, q_len, -1)
        k_channel_logits = (
            q_grouped.unsqueeze(-1)
            * key_states_raw
            .permute(0, 1, 3, 2)
            .unsqueeze(2)
            .unsqueeze(2)
        ...
        k_without_channel = nn.functional.softmax(
            logits_grouped.unsqueeze(3) - k_channel_logits,
            dim=-1,
            dtype=torch.float32,
        ).to(query_states.dtype)
        key_score_raw = (
            k_without_channel - weights_grouped.unsqueeze(3)
        ).pow(2).sum(dim=(-2, -1)).sum(dim=2)
        # upcast attention to fp32
        attn_weights = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
        attn_output = torch.matmul(attn_weights, value_states)

        # Aggregate the contribution of all query heads that share a physical
        # V head. The resulting coordinates map directly to v_proj rows.
        value_score_raw = (
            torch.abs(
                attn_output.reshape(
                    bsz,
        ...
        return (
            ...
            key_score_raw.reshape(bsz, -1).squeeze().tolist(),
            value_score_raw.reshape(bsz, -1).squeeze().tolist(),
