        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    neuron_cap = int(getattr(self.args, "neuron_cap", 100))
                    ...
                    if not bool(getattr(self.args, "kv_indices_are_physical", False)):
                    ...
                    for idx in index_keys:
                        if idx in index_keys_under:
                            activate_fwd_up[idx] = set(itertools.islice(activate_fwd_up[idx], neuron_cap))
                            activate_fwd_down[idx] = set(itertools.islice(activate_fwd_down[idx], neuron_cap))
                            attn_q[idx] = set(itertools.islice(attn_q[idx], neuron_cap))
                            attn_k[idx] = set(itertools.islice(attn_k[idx], neuron_cap))
                            attn_v[idx] = set(itertools.islice(attn_v[idx], neuron_cap))
                    ...
                    parameters_to_mask = (
                        ()
                        if bool(getattr(self.args, "sparse_delta_mode", False))
                        else model.named_parameters()
                    )
                    for name, param in parameters_to_mask:
                        match = re.search(r'layers\.(\d+)\.', name)
                        if match:
                            layer = int(match.group(1))
                            if 'attn.q_proj' in name:
                                tune_index = attn_q[layer]
                                mask = torch.ones(param.size(0), dtype=torch.bool, device=param.grad.device)
                                ...
                                mask = torch.ones(param.size(1), dtype=torch.bool, device=param.grad.device)
                                mask[list(tune_index)] = False
                                param.grad.T[mask] = 0
                            else:
                                param.grad.zero_()
