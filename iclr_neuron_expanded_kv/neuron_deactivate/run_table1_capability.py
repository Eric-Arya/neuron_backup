...
try:
    from .run_table1_deactivated import (
        DEFAULT_NEURONS,
        STRUCTURES,
...
def resolve_dataset_path(data_root: Path, task: str) -> Path:
    """Accept both the historical flat layout and the shared task/config layout."""
    candidates = (
        (data_root / "gsm8k_main", data_root / "gsm8k" / "main")
        if task == "gsm8k"
        else (data_root / "mmlu_all", data_root / "mmlu" / "all")
    )
    for candidate in candidates:
        if candidate.is_dir():
            return candidate
    tried = ", ".join(str(path) for path in candidates)
    raise FileNotFoundError(f"No local {task} dataset found; tried: {tried}")
...
def verify_environment() -> None:
    ...
    if not {"activate_keys_fwd_up_set", "neuron_scale"} <= set(generate_parameters):
        raise RuntimeError("The neuron-scaling Transformers overlay is not installed")
...
def mmlu_prompts(dataset, tokenizer, prompt_format: str) -> list[str]:
    ...
    for record in dataset["test"]:
        ...
        if prompt_format == "chat":
            ...
            demonstrations = "\n\n".join(
                format_mmlu_question(example, True)
                for example in dev_by_subject[subject][:5]
            )
            content = header + demonstrations + "\n\n" + format_mmlu_question(record, False)
...
def evaluate_mmlu(
    ...
    dataset = load_from_disk(str(resolve_dataset_path(args.data_root, "mmlu")))
    test = dataset["test"]
    prompts = mmlu_prompts(dataset, tokenizer, args.prompt_format)
    ...
    for choice in CHOICES:
        # Raw MMLU prompts end at ``Answer:`` and conventionally continue with
        # a space-prefixed option. A Llama-3 chat template instead ends at the
        # assistant header, whose answer content begins with an unprefixed token.
        answer_text = choice if args.prompt_format == "chat" else " " + choice
        encoded = tokenizer.encode(answer_text, add_special_tokens=False)
        if len(encoded) != 1:
    ...
    return {
        ...
        "answer_token_style": (
            "unprefixed assistant content"
            if args.task == "mmlu" and args.prompt_format == "chat"
            else "space-prefixed raw continuation"
            if args.task == "mmlu"
            else None
        ),
        "mmlu_chat_fewshot_style": (
            "multi-turn user/assistant demonstrations"
            if args.task == "mmlu" and args.prompt_format == "chat"
            else None
        ),
...
def gsm8k_prompts(dataset, tokenizer, num_fewshot: int, prompt_format: str) -> list[str]:
    if num_fewshot < 0 or num_fewshot > len(dataset["train"]):
        raise ValueError(
            f"num-fewshot must be in [0, {len(dataset['train'])}], found {num_fewshot}"
        )
    demonstrations = "\n\n".join(
        f"Question: {record['question']}\nAnswer: {record['answer']}"
        for record in dataset["train"].select(range(num_fewshot))
    )
    prompts = []
    for record in dataset["test"]:
        answer_prefix = (
            "Answer: Let's think step by step." if prompt_format == "chat" else "Answer:"
        )
        question = f"Question: {record['question']}\n{answer_prefix}"
        content = f"{demonstrations}\n\n{question}" if demonstrations else question
        if prompt_format == "chat":
            content = tokenizer.apply_chat_template(
...
def evaluate_gsm8k(
    ...
    dataset = load_from_disk(str(resolve_dataset_path(args.data_root, "gsm8k")))
    test = dataset["test"]
    prompts = gsm8k_prompts(dataset, tokenizer, args.num_fewshot, args.prompt_format)
    completed = load_completed(responses_path, fingerprint)
    sample_count = min(len(test), args.limit) if args.limit is not None else len(test)
    expected_ids = set(range(sample_count))
    if set(completed) - expected_ids:
    ...
    with responses_path.open("a", encoding="utf-8") as handle:
        for start in range(0, len(pending), args.batch_size):
            ...
            inputs = tokenizer(
                [prompts[index] for index in indices],
                padding=True,
                # The rendered Llama-3 chat template already starts with BOS.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    count_group.add_argument("--deact-count", type=int)
    count_group.add_argument("--deact-rate", type=float)
    parser.add_argument(
        "--structures",
        nargs="+",
        choices=STRUCTURES,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Multiplier for selected neuron channels (default: 0; examples: 0.5 or 2).",
    )
    parser.add_argument("--layer-scope", choices=("all", "front", "back"), default="all")
    parser.add_argument("--layer-cutoff", type=int)
    ...
    parser.add_argument(
        ...
        choices=("raw", "chat"),
        default="raw",
        help="Prompt rendering (default: raw, retained for prior-run compatibility).",
    )
    parser.add_argument(
        "--num-fewshot",
        type=int,
        default=5,
        help="Number of GSM8K training demonstrations (use 0 for the paper's instruct-model setup).",
    )
...
def main() -> int:
    ...
    if (
        ...
        or args.num_fewshot < 0
        or (args.limit is not None and args.limit <= 0)
        or not math.isfinite(args.neuron_scale)
    ):
        raise SystemExit(
            "batch-size/max-new-tokens must be positive; num-fewshot/limit must be valid; "
            "neuron-scale must be finite"
        )
    if args.task == "mmlu" and args.num_fewshot != 5:
        raise SystemExit("MMLU currently uses its standard 5-shot dev split")
    ...
    if args.task == "gsm8k" and args.num_fewshot != 5:
        run_config["num_fewshot"] = args.num_fewshot
    ...
    variant_tag = args.variant if args.variant == "origin" else f"{args.variant}_{intervention_tag}"
    if args.variant != "origin" and args.neuron_scale != 0:
        scale_tag = format(args.neuron_scale, ".12g").replace("-", "neg").replace(".", "p")
        variant_tag += f"_scale{scale_tag}"
    if args.prompt_format != "raw" or (args.task == "gsm8k" and args.num_fewshot != 5):
        variant_tag += f"_{args.prompt_format}_{args.num_fewshot}shot"
