...
class LlamaMLP(nn.Module):
    ...
    def _scale_channels(self, states, indices, scale, cache_name):
        if not indices or scale == 1:
            return states
        ...
        if (
            ...
            or cached[1] != scale
            or cached[2].device != states.device
            or cached[2].dtype != states.dtype
        ):
            multiplier = states.new_ones(self.intermediate_size)
            multiplier[list(signature)] = scale
            cached = (signature, scale, multiplier)
            setattr(self, cache_name, cached)
        return states * cached[2]

    def forward(
        self,
        x,
        deactivate_keys_fwd_up,
        deactivate_keys_fwd_down,
        neuron_scale=0.0,
    ):
        ...
        up_proj = self._scale_channels(
            up_proj, deactivate_keys_fwd_up, neuron_scale, "_scaled_fwd_up_multiplier"
        )
        intermediate_states = self.act_fn(gate_proj) * up_proj
        intermediate_states = self._scale_channels(
            intermediate_states,
            deactivate_keys_fwd_down,
            neuron_scale,
            "_scaled_fwd_down_multiplier",
        )

        if self.config.pretraining_tp > 1:
            intermediate_slices = intermediate_states.split(slice, dim=2)
            down_proj = [
...
class LlamaAttention(nn.Module):
    ...
    def _scale_channels(self, states, indices, scale, cache_name, flattened=False):
        """Scale selected projection or replica-specific expanded-GQA channels."""
        if not indices or scale == 1:
            return states
        signature = tuple(sorted(indices))
        cached = getattr(self, cache_name, None)
        if (
            ...
            multiplier = states.new_ones(self.num_heads * self.head_dim)
            multiplier[list(signature)] = scale
            if flattened:
                multiplier = multiplier.view(1, self.num_heads, 1, self.head_dim)
            cached = (signature, scale, multiplier)
            setattr(self, cache_name, cached)
        return states * cached[2]
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states = self._scale_channels(
            query_states, deactivate_keys_q, neuron_scale, "_scaled_q_multiplier"
        )
        ...
        key_states = self._scale_channels(
            key_states, deactivate_keys_k, neuron_scale, "_scaled_expanded_k_multiplier", True
        )
        value_states = self._scale_channels(
            value_states, deactivate_keys_v, neuron_scale, "_scaled_expanded_v_multiplier", True
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mlp(
            hidden_states, deactivate_keys_fwd_up, deactivate_keys_fwd_down, neuron_scale
        )
        hidden_states = residual + hidden_states
