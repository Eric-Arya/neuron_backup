...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x, early_layers):
        if self.config.pretraining_tp > 1:
            ...
            down_proj = [
                F.linear(intermediate_states.split(slice, dim=2)[i], down_proj_slices[i])
                for i in range(self.config.pretraining_tp)
            ]
            real_output = sum(down_proj)
        else:
            up_proj = self.up_proj(x)
            intermediate_states = self.act_fn(self.gate_proj(x)) * up_proj
            real_output = self.down_proj(intermediate_states)


        if early_layers:
            # Appendix A.1, Eq. (9): removing intermediate coordinate k changes
            # the FFN output by h_ffn[:, k] outer W_down[:, k].  Its L2 norm
            # factorizes into the two vector norms below.  W_up row k and
            # W_down column k describe the same intermediate neuron, so they
            # intentionally receive the same score.
            intermediate_norm = torch.linalg.vector_norm(
                intermediate_states.float(), dim=1
            )
            down_column_norm = torch.linalg.vector_norm(
                self.down_proj.weight.float(), dim=0
            )
            ffn_score = intermediate_norm * down_column_norm
            score = ffn_score.squeeze().tolist()
            return real_output, score, score
...
class LlamaAttention(nn.Module):
    ...
    def shared_forward(
        ...
        **kwargs,
        ...
        # Estimate the effect of removing each query channel independently.
        q_channel_logits = (
            query_states.transpose(2, 3).unsqueeze(-1)
            * key_states.transpose(2, 3).unsqueeze(-2)
            / math.sqrt(self.head_dim)
        )
        q_without_channel = nn.functional.softmax(
            attn_logits.unsqueeze(2) - q_channel_logits,
            dim=-1,
            dtype=torch.float32,
        ).to(query_states.dtype)
        query_score = (q_without_channel - attn_weights.unsqueeze(2)).pow(2).sum(dim=(-2, -1))

        # Appendix A.1 treats Q and K symmetrically.  For Llama-3 GQA we apply
        # that equation after repeat_kv, retaining one independently maskable
        # K coordinate for each query-head replica (4096 coordinates total).
        key_score = query_score
        ...
        # V is linear before o_proj, so Eq. (9) applies directly in the
        # expanded 4096-coordinate space.
        value_score = torch.linalg.vector_norm(attn_output.float(), dim=1)
        value_score = value_score * torch.linalg.vector_norm(
            self.o_proj.weight.float(), dim=0
        )
        ...
        return (
            ...
            attn_weights,
            past_key_value,
            query_score.reshape(bsz, -1).squeeze().tolist(),
            key_score.reshape(bsz, -1).squeeze().tolist(),
            value_score.reshape(bsz, -1).squeeze().tolist(),
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output, attn_weights, past_key_value_real, query_score, key_score, value_score, o_score = self.shared_forward(
        ...
        if early_layers:
            return attn_output, attn_weights, past_key_value_real, query_score, key_score, value_score, o_score
        else:
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        early_exit_layers = early_exit_layers or []
    ...
    def forward(
        ...
        if early_exit_layers is not None:
            ...
            for i, early_exit_layer in enumerate(early_exit_layers):
                ...
                top_number_attn = int(getattr(self.config, "safety_neuron_top_attn", 100))
                top_number_ffn = int(getattr(self.config, "safety_neuron_top_ffn", 200))
                top_number_layer = 10
                # pdb.set_trace()
