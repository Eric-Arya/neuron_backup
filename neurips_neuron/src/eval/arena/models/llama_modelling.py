...
class LlamaModelForScore(ScoreModelMixin, LlamaPreTrainedModel):
    ...
    def __init__(self, config: PretrainedConfig, **kwargs: Any) -> None:
        ...
        # Beaver checkpoints use ``score_bias`` while newer safe-rlhf code
        # consults ``bias``. Honor the checkpoint field so every score-head
        # tensor is loaded (the released cost checkpoint's bias is zero).
        kwargs.setdefault("bias", getattr(config, "score_bias", getattr(config, "bias", False)))
