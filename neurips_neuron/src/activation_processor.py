...
def scale_hooked_ia3_displacement_(model: HookedPreTrainedModel, alpha: float) -> int:
    """Scale IA3 tensors around their identity value of one."""
    if not torch.isfinite(torch.tensor(alpha)) or alpha < 0:
        raise ValueError("IA3 displacement alpha must be finite and nonnegative")
    scaled_parameters = 0
    if alpha == 1:
    ...
    if scaled_parameters == 0:
        raise ValueError("No IA3 tensors found in hooked model")
    return scaled_parameters
    ...
    def process_prompts(self, model: HookedPreTrainedModel, prompts: list[str], token_type: str):
        ...
        if 'prompt' in token_type:
            for i in tqdm(range(0, len(prompts), self.batchsize), desc="Processing prompts"):
                ...
                prepend_bos = not (bos and all(prompt.startswith(bos) for prompt in batch_prompts))
                tokenized_prompts = model.to_tokens(
                    batch_prompts, prepend_bos=prepend_bos, device=model.device
                )
...
class ActivationContrasting(BaseActivationProcessor):
    def __init__(self, base_model_name_or_path: str, first_peft_path: list[str], second_peft_path: list[str], batchsize=20, max_new_tokens=128, tokenizer_name_or_path=None, first_ia3_alpha=1.0, second_ia3_alpha=1.0, **load_parameter) -> None:
        """  
        Args:
        ...
        self.tokenizer_name_or_path = tokenizer_name_or_path or base_model_name_or_path
        self.first_peft_path = first_peft_path
        self.second_peft_path = second_peft_path
        self.first_ia3_alpha = first_ia3_alpha
        self.second_ia3_alpha = second_ia3_alpha
    ...
    def compute_change_scores(self,
        ...
        scale_hooked_ia3_displacement_(hooked_model, self.second_ia3_alpha)
        ...
        scale_hooked_ia3_displacement_(hooked_model, self.first_ia3_alpha)
        hooked_model.set_tokenizer(tokenizer)
