        ...
        for idx, example in enumerate(examples):
            ...
            for message in example["conversations"]:
                if message["from"] == "human" or message["from"] == "user":
                    ...
                    invalid_cnt += 1
                    break
            # Keep only complete alternating user/assistant conversations.
            # This protects training from malformed records when raw data is
            # supplied without the tokenizer-based splitter.
            if valid and (
                not messages
                or len(messages) % 2 != 0
                or any(
