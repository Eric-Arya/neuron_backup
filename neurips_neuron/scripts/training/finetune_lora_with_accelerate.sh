#!/usr/bin/env bash
# Example: CUDA_VISIBLE_DEVICES=0,1 NUM_GPUS=2 BATCH_SIZE_PER_GPU=2 bash scripts/training/finetune_lora_with_accelerate.sh
set -euo pipefail
...
data_root=${DATA_ROOT:-/data1/cjh/Alignment}
# Pass MODEL_PATH to train any AutoModelForCausalLM checkpoint.
...
TOTAL_BATCH_SIZE=${TOTAL_BATCH_SIZE:-120}
GRADIENT_ACC_STEPS=${GRADIENT_ACC_STEPS:-$((TOTAL_BATCH_SIZE / NUM_GPUS / BATCH_SIZE_PER_GPU))}
if (( GRADIENT_ACC_STEPS < 1 )); then
    echo "TOTAL_BATCH_SIZE must be at least NUM_GPUS * BATCH_SIZE_PER_GPU" >&2
    exit 2
fi
...
do
    ...
    do
        ...
        accelerate launch \
            ...
            --weight_decay 0.1 \
            --num_train_epochs 3 \
            --output_dir ${data_root}/output/${MODEL_LABEL}_${save_name[i]}_ia3_ff_${seed} \
            --report_to wandb \
            --with_tracking \
            --logging_steps 10 \
